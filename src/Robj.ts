...
const Robj = {
  ...
  clos: function (formals: any, body: any, attributes: any) {
    return {
      type: "clos",
  ...
  tagged_list: make_basic("tagged_list", {
    json: function (resolver) {
      function classify_list(list: any) {
        if (
      ...
      var list: any = this.value.slice(1);
  ...
  int_array: make_basic("int_array", {
    json: function () {
      if (
        ...
        var arr: any = _.map(this.value, function (factor) {
