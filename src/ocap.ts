...
function _encode_command(command, buffer, msg_id) {
  ...
  var length = _.reduce(
    ...
    view = new EndianAwareDataView(big_buffer);
...
const create = function (opts) {
  ...
  function _encode_value(value, forced_type?: any) {
    var sz = determine_size(value, forced_type);
  ...
  function hand_shake(msg) {
    ...
    if (typeof msg === "string") {
      ...
      if (header === "RsOC") {
        ...
        result.bare_ocap = parse_payload(msg)?.value;
  ...
  socket.onmessage = function (m) {
    ...
    const msg =
      m.data.constructor.name === "Buffer"
    ...
    var queue: any = _.find(queues, function (queue) {
    ...
    if (!v.ok) {
      ...
      queue = Rsrv.OOB_USR_CODE(cmd) > 255 ? compute_queue : ctrl_queue;
      ...
      if (_.isString(p[0])) {
        if (_.isUndefined(opts.on_oob_message)) {
          ...
          opts.on_oob_message(v.payload, function (error, result) {
            ...
            if (error) {
              _send_cmd_now(cmd | Rsrv.RESP_ERR, _encode_string(error), msg_id);
            } else {
              _send_cmd_now(cmd | Rsrv.RESP_OK, _encode_string(result), msg_id);
            }
            bump_queues();
          });
        }
        ...
        if (!result.ocap_mode) {
          ...
          params.push(function (err, result) {
            if (err) {
              _send_cmd_now(Rsrv.RESP_ERR | cmd, _encode_value(err), msg_id);
            } else {
  ...
  function _send_cmd_now(command, buffer, msg_id?: any) {
  ...
  function bump_queues() {
    ...
    if (result.closed) {
      ...
      var queue: any = _.sortBy(available, function (queue) {
  ...
  function _cmd(command, buffer, k, string, queue?: any) {
...
interface RObject extends Array<any> {
  r_type: string;
  r_attributes: {
    class: string;
    [key: string]: any;
...
interface RArraryBuffer extends ArrayBuffer {
  [key: string]: any;
}

type Replicate = RObject | TypedArray | Function | RArraryBuffer;

const wrap_all_ocaps = function (s, v) {
  v = v.value.json(s.resolve_hash);
  function replace(obj: Replicate) {
    let result: any = obj;
    if (
      _.isArray(obj) &&
      "r_attributes" in obj &&
...
const wrap_ocap = function (s, ocap) {
  var wrapped_ocap: any = function () {
    ...
    s.OCcall(ocap, values, function (err, v) {
      if (!_.isUndefined(v)) v = wrap_all_ocaps(s, v);
      k(err, v);
    });
  };
...
export { create, wrap_all_ocaps, wrap_ocap };
