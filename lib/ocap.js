...
exports.wrap_ocap = exports.wrap_all_ocaps = exports.create = void 0;
...
function _encode_command(command, buffer, msg_id) {
    if (!underscore_1.default.isArray(buffer))
        buffer = [buffer];
    if (!msg_id)
        msg_id = 0;
    var length = underscore_1.default.reduce(buffer, function (memo, val) {
        return memo + val.byteLength;
    }, 0), big_buffer = new ArrayBuffer(16 + length), view = new endian_1.EndianAwareDataView(big_buffer);
    view.setInt32(0, command);
    view.setInt32(4, length);
    view.setInt32(8, msg_id);
    view.setInt32(12, 0);
    var offset = 16;
    underscore_1.default.each(buffer, function (b) {
...
function _encode_string(str) {
    ...
    var view = new endian_1.EndianAwareDataView(result);
    view.setInt32(0, Rsrv_1.default.DT_STRING + (strl << 8));
...
function _encode_bytes(bytes) {
    ...
    var view = new endian_1.EndianAwareDataView(result);
    view.setInt32(0, Rsrv_1.default.DT_BYTESTREAM + (payload_length << 8));
    for (var i = 0; i < bytes.length; ++i)
        view.setInt8(4 + i, bytes[i]);
    return result;
}
const create = function (opts) {
    opts = underscore_1.default.defaults(opts || {}, {
    ...
    var handle_error = opts.on_error ||
        function (error) {
            throw new error_1.default(error, -1);
        };
    ...
    function _encode_value(value, forced_type) {
        var sz = (0, write_1.determine_size)(value, forced_type);
        // all this will still break if sz is, say, >= 2^31.
        if (sz > 16777215) {
            var buffer = new ArrayBuffer(sz + 8);
            var view = (0, endian_1.my_ArrayBufferView)(buffer);
            // can't left shift value here because value will have bit 32 set and become signed..
            view
                .data_view()
                .setInt32(0, Rsrv_1.default.DT_SEXP + (sz & 16777215) * Math.pow(2, 8) + Rsrv_1.default.DT_LARGE);
            // but *can* right shift because we assume sz is less than 2^31 or so to begin with
            view.data_view().setInt32(4, sz >>> 24);
            (0, write_1.write_into_view)(value, view.skip(8), forced_type, convert_to_hash);
            return buffer;
        }
        else {
            var buffer = new ArrayBuffer(sz + 4);
            var view = (0, endian_1.my_ArrayBufferView)(buffer);
            view.data_view().setInt32(0, Rsrv_1.default.DT_SEXP + (sz << 8));
            (0, write_1.write_into_view)(value, view.skip(4), forced_type, convert_to_hash);
            return buffer;
        }
    }
    function hand_shake(msg) {
        ...
        if (typeof msg === "string") {
            ...
            if (header === "RsOC") {
                ...
                result.bare_ocap = (0, read_1.parse_payload)(msg)?.value;
                result.ocap = wrap_ocap(result, result.bare_ocap);
                result.running = true;
                onconnect && onconnect.call(result);
            }
            else
    ...
    socket.onmessage = function (m) {
        ...
        const msg = m.data.constructor.name === "Buffer"
            ? new MessageEvent(m.type, {
                data: new Uint8Array(m.data).buffer,
                origin: m.origin,
                lastEventId: m.lastEventId,
                source: m.source,
                ports: Array.from(m.ports),
            })
            : m;
        if (opts.debug)
        ...
        var v = (0, read_1.parse)(msg.data);
        if (v.incomplete)
            return;
        var msg_id = v.header[2], cmd = v.header[0] & 0xffffff;
        var queue = underscore_1.default.find(queues, function (queue) {
        ...
        if (!v.ok) {
        ...
        else if (cmd === Rsrv_1.default.RESP_OK) {
            queue.result_callback(null, v.payload);
        }
        else if (Rsrv_1.default.IS_OOB_SEND(cmd)) {
            opts.on_data && opts.on_data(v.payload);
        }
        else if (Rsrv_1.default.IS_OOB_MSG(cmd)) {
            ...
            queue = Rsrv_1.default.OOB_USR_CODE(cmd) > 255 ? compute_queue : ctrl_queue;
            // console.log("OOB MSG result on queue "+ queue.name);
            var p;
            try {
                p = wrap_all_ocaps(result, v.payload); // .value.json(result.resolve_hash);
            }
            catch (e) {
                _send_cmd_now(Rsrv_1.default.RESP_ERR | cmd, _encode_string(String(e)), msg_id);
                return;
            }
            if (underscore_1.default.isString(p[0])) {
                if (underscore_1.default.isUndefined(opts.on_oob_message)) {
                    _send_cmd_now(Rsrv_1.default.RESP_ERR | cmd, _encode_string("No handler installed"), msg_id);
                }
                else {
                    ...
                    opts.on_oob_message(v.payload, function (error, result) {
                        ...
                        if (error) {
                            _send_cmd_now(cmd | Rsrv_1.default.RESP_ERR, _encode_string(error), msg_id);
                        }
                        else {
                            _send_cmd_now(cmd | Rsrv_1.default.RESP_OK, _encode_string(result), msg_id);
                        }
                        bump_queues();
                    });
                }
            }
            else if (underscore_1.default.isFunction(p[0])) {
                if (!result.ocap_mode) {
                    _send_cmd_now(Rsrv_1.default.RESP_ERR | cmd, _encode_string("JavaScript function calls only allowed in ocap mode"), msg_id);
                }
                else {
                    var captured_function = p[0], params = p.slice(1);
                    params.push(function (err, result) {
                        if (err) {
                            _send_cmd_now(Rsrv_1.default.RESP_ERR | cmd, _encode_value(err), msg_id);
                        }
                        else {
                ...
                _send_cmd_now(Rsrv_1.default.RESP_ERR | cmd, _encode_string("Unknown oob message type: " + typeof p[0]));
            }
        }
        else {
    ...
    function queue_can_send(queue) {
        return (!queue.in_oob_message && !queue.awaiting_result && queue.queue.length);
    }
    function bump_queues() {
        var available = underscore_1.default.filter(queues, queue_can_send);
        ...
        if (result.closed) {
            ...
            var queue = underscore_1.default.sortBy(available, function (queue) {
    ...
    result = {
        ...
        login: function (command, k) {
            _cmd(Rsrv_1.default.CMD_login, _encode_string(command), k, command);
        },
        eval: function (command, k) {
            _cmd(Rsrv_1.default.CMD_eval, _encode_string(command), k, command);
        },
        createFile: function (command, k) {
            _cmd(Rsrv_1.default.CMD_createFile, _encode_string(command), k, command);
        },
        writeFile: function (chunk, k) {
            _cmd(Rsrv_1.default.CMD_writeFile, _encode_bytes(chunk), k, "");
        },
        closeFile: function (k) {
            _cmd(Rsrv_1.default.CMD_closeFile, new ArrayBuffer(0), k, "");
        },
        set: function (key, value, k) {
            _cmd(Rsrv_1.default.CMD_setSEXP, [_encode_string(key), _encode_value(value)], k, "");
        },
        //////////////////////////////////////////////////////////////////////
        // ocap mode
        OCcall: function (ocap, values, k) {
            ...
            try {
                // replaced |= with ||= as this is more appropriate for boolean values
            ...
            _cmd(Rsrv_1.default.CMD_OCcall, _encode_value(params, Rsrv_1.default.XT_LANG_NOTAG), k, "", queue);
        },
        wrap_ocap: function (ocap) {
            return wrap_ocap(this, ocap);
        },
...
const wrap_all_ocaps = function (s, v) {
    ...
    function replace(obj) {
        ...
        else if (underscore_1.default.isArray(obj)) {
            result = underscore_1.default.map(obj, replace);
            result.r_type = obj.r_type;
            result.r_attributes = obj.r_attributes;
        }
        else if (underscore_1.default.isTypedArray(obj)) {
            return obj;
        }
        else if (underscore_1.default.isFunction(obj)) {
            return obj;
        }
        else if (obj &&
            !underscore_1.default.isUndefined(obj.byteLength) &&
            !underscore_1.default.isUndefined(obj.slice)) {
            // ArrayBuffer
            return obj;
        }
        else if (underscore_1.default.isObject(obj)) {
            result = underscore_1.default.object(underscore_1.default.map(obj, function (v, k) {
                return [k, replace(v)];
            }));
        }
        return result;
...
exports.wrap_all_ocaps = wrap_all_ocaps;
const wrap_ocap = function (s, ocap) {
    var wrapped_ocap = function () {
        var values = underscore_1.default.toArray(arguments);
        // common error (tho this won't catch the case where last arg is a function)
        if (!values.length || !underscore_1.default.isFunction(values[values.length - 1]))
            throw new Error("forgot to pass continuation to ocap");
        var k = values.pop();
        s.OCcall(ocap, values, function (err, v) {
            if (!underscore_1.default.isUndefined(v))
                v = wrap_all_ocaps(s, v);
            k(err, v);
        });
    };
